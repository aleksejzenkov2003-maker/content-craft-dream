    ...
    >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              ...
              <div className="relative aspect-[9/16] bg-muted rounded-lg overflow-hidden border">
                {sceneUrls.length > 0 ? (
                  <>
                    <img src={sceneUrls[currentVariant] || sceneUrls[0]} alt="Scene" className="w-full h-full object-cover" />
                    {status === 'approved' && (
                    ...
                    {sceneUrls.length > 1 && (
                      <>
                        ...
                        <button onClick={() => setCurrentVariant(Math.min(sceneUrls.length - 1, currentVariant + 1))} className="absolute right-1 top-1/2 -translate-y-1/2 w-8 h-8 bg-background/80 rounded-full flex items-center justify-center" disabled={currentVariant === sceneUrls.length - 1}>
        ...
        <TabsContent value="prompt" className="mt-4 space-y-3">
          <Textarea value={scene.scene_prompt || ''} onChange={(e) => handlePromptChange(e.target.value)} placeholder="Опишите желаемую сцену..." rows={8} className="resize-none text-sm" />
